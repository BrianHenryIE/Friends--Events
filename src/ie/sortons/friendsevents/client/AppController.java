			...
			Canvas.getPageInfo(new AsyncCallback<PageInfo>() {
				...
				public void onSuccess(PageInfo info) {
					...
					if (info.getScrollTop() < info.getOffsetTop()) {
						...
						RootPanel.get("gwt").setHeight(info.getClientHeight() - info.getOffsetTop() + info.getScrollTop() + "px");
					} else {
						RootPanel.get("gwt").setHeight(info.getClientHeight() + "px");
						RootPanel.get("gwt").getElement().getStyle().setMarginTop(info.getScrollTop() - info.getOffsetTop(), Unit.PX);
					}
